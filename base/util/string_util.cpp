...
namespace detail
{
	std::wstring SysMultiByteToWide(std::string mb, uint32_t code_page) {
		if (mb.empty())
			return std::wstring();
...
{
	auto wide= detail::SysMultiByteToWide(ansi, CP_ACP);
	return detail::SysWideToMultiByte(wide, CP_UTF8);
}

std::wstring AnsiToUnicode(const std::string& strAnsi)
{
	return detail::SysMultiByteToWide(strAnsi, CP_ACP);
}

std::string UnicodeToAnsi(const std::wstring& wide)
{
	return detail::SysWideToMultiByte(wide, CP_ACP);
}
