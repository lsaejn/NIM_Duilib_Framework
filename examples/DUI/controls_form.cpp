...
void ControlForm::InitWindow()
{
	 ...
	 */
	ui::ListBox* listh = dynamic_cast<ui::ListBox*>(FindControl(L"hlist"));
	for (auto i = 0; i < 5; i++)
	{
		...
		//btn->SetBkColor(L"red");
		btn->SetText(nbase::StringPrintf(L"ListElement%d", i));
		btn->SetAttribute(L"normaltextcolor", L"white");
		btn->SetAttribute(L"pushedtextcolor", L"dark_gray");
		btn->SetAttribute(L"hottextcolor", L"green");
		btn->SetAttribute(L"margin", L"3,3,3,100");
		btn->SetAttribute(L"width", L"80");
		btn->SetFont(L"system_16");
		/*
